...
from tools import get_embedding_new, get_split_new, get_config, logWritter, MeaninglessError, scores_gzsl, Step_Scheduler, Const_Scheduler, construct_gt_st
...
def parse_args():
    ...
    parser.add_argument('--ngpu', type=int, default=1, help='number of GPUs to be used if multigpus is Ture, GPU id otherwise')
    ...
    parser.add_argument(
        "--seen_classes_idx_metric", type=int, default=[]
    )
    parser.add_argument(
        "--unseen_classes_idx_metric", type=int, default=[]
    )
    # keep empty
    parser.add_argument("--unseen_classes_idx", type=int, default=[])
    parser.add_argument("--num_unseen_class", type=int, default=10)
    parser.add_argument("--gpu", type=int, default=3)
    args = parser.parse_args()
    unseen_classes_dic = {2: [10, 14], 4: [10, 14, 1, 18],
                          6: [10, 14, 1, 18, 8, 20],
                          8: [10, 14, 1, 18, 8, 20, 19, 5],
                          10: [10, 14, 1, 18, 8, 20, 19, 5, 9, 16]}
    unseen_classes_idx_metric = unseen_classes_dic[args.num_unseen_class]
    ...
    args.unseen_classes_idx = unseen_classes_idx_metric
...
def main():
    ...
    Get dataLoader
    """
    vals_cls, valu_cls, all_labels, visible_classes, visible_classes_test, cls_map, cls_map_test = get_split_new(config, args)
    ...
    Get trainer
    """
    args.batch_size = config['BATCH_SIZE']['TRAIN']
    kwargs = {"num_workers": 8, "pin_memory": True}
    (loader, loader_test, _, _,) = make_data_loader(
        args, load_embedding=False, w2c_size=300, **kwargs
    )
    class_emb = get_embedding_new(args)
    class_emb_vis = class_emb[visible_classes]
    class_emb_vis_ = torch.zeros((config['ignore_index'] + 1 - class_emb_vis.shape[0], class_emb_vis.shape[1]),
                                 dtype=torch.float32)
    class_emb_vis_aug = torch.cat((class_emb_vis, class_emb_vis_), dim=0)
    class_emb_all = class_emb[visible_classes_test]
