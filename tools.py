...
def get_embedding_new(args):
    if args.dataset == "pascal":
        emb = np.load("embeddings/context/pascalcontext_class_w2c.npy")
    class_emb = torch.from_numpy(emb)
    return class_emb
...
def get_split_new(cfg, args):
    seen_classes = np.array(args.seen_classes_idx_metric).astype(np.int32)
    novel_classes = np.array(args.unseen_classes_idx).astype(np.int32)
    seen_novel_classes = np.concatenate((seen_classes, novel_classes), axis=0)
    all_labels = np.array(CLASSES_NAMES)
    if args.dataset == "pascal":
        all_labels = all_labels[:21]
    ...
    return seen_classes, novel_classes, all_labels, visible_classes, visible_classes_test, cls_map, cls_map_test
